...
export async function onRequestGet(context) {
  ...
  try {
    const row = await context.env.DB.prepare(
      'SELECT settings_json, last_video_json, last_playlist_json, updated_at FROM app_state WHERE client_id = ?1'
    ...
    return jsonResponse({
      ...
      lastPlaylist: parseJson(row && row.last_playlist_json, null),
...
export async function onRequestPatch(context) {
  ...
  try {
    ...
    const lastVideo = Object.prototype.hasOwnProperty.call(body, 'lastVideo')
      ...
      : null;
    const lastPlaylist = Object.prototype.hasOwnProperty.call(body, 'lastPlaylist')
      ? JSON.stringify(body.lastPlaylist || null)
      : null;
    const now = Date.now();

    await context.env.DB.prepare(`
      INSERT INTO app_state (client_id, settings_json, last_video_json, last_playlist_json, updated_at)
      VALUES (?1, ?2, ?3, ?4, ?5)
      ON CONFLICT(client_id) DO UPDATE SET
        ...
        last_playlist_json = CASE WHEN ?4 IS NULL THEN app_state.last_playlist_json ELSE ?4 END,
        updated_at = ?5
    `).bind(session.id, settings, lastVideo, lastPlaylist, now).run();
